...
class GetTelemetryPackTool extends MCPTool<Input, typeof schema> {
  name = "tdf_group_telemetry";
  description =
    "Tour de France live telemetry for on-course groups. Empty until the stage is racing.";
  schema = schema;

  async execute({ year, stage, limit, offset }: Input) {
    const rows = (await apiGet(`/api/telemetryPack-${year}-${stage}`)) as unknown[];
    return paginate(rows, limit, offset);
  }
}

export default GetTelemetryPackTool;
